...
- **即点即译（X 式 lazy translate, hechun 自作）— `apps/api/plane/app/views/issue/comment.py`**
  ...
  - **★富文本「排版一模一样」翻译（2026-06-10 hechun）**：旧实现前端 tokenize 把 HTML 平文化（段落以外的 色/标题/列表/加粗 全 strip）→ 译文扁平。新实现 = **服务端 DOM 结构保持**：ai-bot `translate_html()`（bs4 解析 → 只翻 DOM 文本节点 → \n 连结一次翻译[hy-mt2 保 \n 1:1]→ \n 分割回填同一棵 DOM → 序列化；计数不符逐节点兜底）。标签/属性/颜色/标题/列表/图片/@mention **一字节不动**。`/translate` 端点加 `{html}` 模式。**两路统一**：on-demand(`CommentTranslateOnDemandEndpoint` 主路改成读 `comment_html` → 调 ai-bot `/translate{html}` → 缓存 HTML) + prewarm(`_maybe_autotranslate` 改用 translate_html) 都按 **`source_hash=sha256(comment_html)`** 键（`CommentTranslationUpsertAPIEndpoint` 同步设 source_hash）→ prewarm 命中 on-demand。前端 `display.tsx` 删 tokenize/rebuild，只发 `{target_lang}`、直接渲染返回的结构保持 HTML。诚实边界：行内夹断（一句话中间几个字加粗/变色）结构保留但翻译质量略降（片段失上下文，CS 评论少见）。依赖：ai-bot venv 加 `beautifulsoup4`

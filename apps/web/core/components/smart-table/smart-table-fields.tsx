...
const SELECTABLE: TSmartColumnType[] = ["single_select", "multi_select"];
...
export function SmartTableFields({ ws, pid, table, focusColumnId }: Props) {
  ...
  useEffect(() => { setNameDraft(sel?.name ?? ""); setConfirmDel(false); }, [selectedId, sel?.name]);

  const T = zh
    ? { hint: "左边选字段,右边改名称 / 类型 / 选项 / 必填;↑↓ 调列顺序。", add: "添加字段", empty: "还没有字段", name: "名称", type: "类型", required: "必填字段", reqHint: "卡片完成时未填会标「待补全」", options: "选项", addOpt: "添加选项", del: "删除字段", delConfirm: "确认删除?数据会丢", derived: "派生列 · 只读投影", derivedHint: "值由卡片 / AI / keiri 自动算出,不可改类型;可改显示名或删除。", manual: "手动", readonly: "只读" }
    : { hint: "左でフィールド選択 → 右で名称 / 種類 / 選択肢 / 必須を編集;↑↓ で並び替え。", add: "フィールド追加", empty: "フィールドがありません", name: "名称", type: "種類", required: "必須フィールド", reqHint: "カード完了時に未入力なら「未補完」に", options: "選択肢", addOpt: "選択肢を追加", del: "フィールド削除", delConfirm: "削除しますか?データは消えます", derived: "派生列 · 読取専用", derivedHint: "値はカード / AI / keiri が自動算出。種類は変更不可;表示名変更と削除は可。", manual: "手動", readonly: "読取専用" };

  const patchCol = useCallback((cid: string, patch: Partial<TSmartColumn>) => {
  ...
  const move = useCallback((i: number, dir: -1 | 1) => {
    ...
    [re[i], re[j]] = [re[j], re[i]];
    setCols(re);
    void Promise.all(re.map((c, k) => smartTableService.updateColumn(ws, pid, table.id, c.id, { position: k }).catch(() => {})));
  ...
  const addField = useCallback(async () => {
    ...
    if (c) { setCols((prev) => [...prev, c]); setSelectedId(c.id); }
      ...
      <div className="flex w-72 shrink-0 flex-col border-r border-subtle bg-layer-1">
        ...
        <div className="vertical-scrollbar flex-1 overflow-auto p-1.5">
          ...
          {cols.map((c, i) => {
            ...
            return (
              <div key={c.id} className={cn("group mb-0.5 flex items-center gap-1.5 rounded-sm pr-1.5", isSel ? "bg-layer-2" : "hover:bg-layer-1-hover")}>
                <div className="flex flex-col opacity-0 group-hover:opacity-100">
                  <button type="button" disabled={i === 0} onClick={() => move(i, -1)} className="flex h-3.5 w-5 items-center justify-center text-tertiary hover:text-secondary disabled:opacity-30"><ChevronUp className="size-3" /></button>
                  <button type="button" disabled={i === cols.length - 1} onClick={() => move(i, 1)} className="flex h-3.5 w-5 items-center justify-center text-tertiary hover:text-secondary disabled:opacity-30"><ChevronDown className="size-3" /></button>
                </div>
                <button type="button" onClick={() => setSelectedId(c.id)} className="flex min-w-0 grow items-center gap-2 py-1.5 text-left">
                  <Icon className={cn("size-3.5 shrink-0", isSel ? "text-accent-primary" : "text-tertiary")} />
                  <span className={cn("truncate text-13", isSel ? "font-medium text-primary" : "text-secondary")}>{c.name}</span>
        ...
        <button type="button" onClick={addField} className="flex items-center gap-1.5 border-t border-subtle px-4 py-2.5 text-13 font-medium text-accent-primary hover:bg-layer-1-hover">
      ...
      <div className="vertical-scrollbar min-w-0 flex-1 overflow-auto">
        {!sel ? (
          ...
          <div className="mx-auto max-w-md px-6 py-6">
            {/* 名称 */}
            <div className="mb-1 text-11 font-semibold uppercase tracking-wide text-placeholder">{T.name}</div>
            <input
              value={nameDraft}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setNameDraft(e.target.value)}
              onBlur={() => { const n = nameDraft.trim(); if (n && n !== sel.name) patchCol(sel.id, { name: n }); }}
            ...
            {sel.source === "manual" ? (
              <>
                {/* 类型: 图标+文字 网格 */}
                <div className="mb-1 mt-5 text-11 font-semibold uppercase tracking-wide text-placeholder">{T.type}</div>
                <div className="grid grid-cols-2 gap-1.5">
                  {TYPE_META.map(({ v, zh: z, ja, Icon }) => (
                    <button key={v} type="button" onClick={() => patchCol(sel.id, { type: v })} className={cn("flex items-center gap-2 rounded-md border px-2.5 py-2 text-13", sel.type === v ? "border-accent-strong bg-accent-subtle text-accent-primary" : "border-subtle text-secondary hover:bg-layer-1-hover")}>
                ...
                <label className="mt-5 flex cursor-pointer items-start gap-2.5">
                  <input type="checkbox" checked={sel.required} onChange={(e: ChangeEvent<HTMLInputElement>) => patchCol(sel.id, { required: e.target.checked })} className="mt-0.5 size-4" />
                  <span>
                ...
                {/* 选项(单选/多选) */}
                {SELECTABLE.includes(sel.type) && (
                  <>
                    <div className="mb-1.5 mt-5 text-11 font-semibold uppercase tracking-wide text-placeholder">{T.options}</div>
                    <div className="flex flex-wrap items-center gap-1.5 rounded-md border border-subtle p-2.5">
                      {(sel.options ?? []).map((o, i) => (
                        <span key={i} className="inline-flex items-center gap-1 rounded-full py-0.5 pl-1 pr-1.5" style={{ background: (o.color || "#7a8088") + "26", border: `1px solid ${(o.color || "#7a8088") + "55"}` }}>
                          <button
                            type="button"
                            title={zh ? "换颜色" : "色を変更"}
                            onClick={() => {
                          ...
                          />
                          <button type="button" onClick={() => patchCol(sel.id, { options: (sel.options ?? []).filter((_, j) => j !== i) })} className="text-tertiary hover:text-danger-primary">
                            <X className="size-3" />
                          </button>
                        </span>
                      ))}
                      <button
                        type="button"
                        onClick={() => patchCol(sel.id, { options: [...(sel.options ?? []), { v: (zh ? "选项 " : "選択肢 ") + ((sel.options?.length ?? 0) + 1), color: PALETTE[(sel.options?.length ?? 0) % PALETTE.length] } as TSmartSelectOption] })}
            ...
            <div className="mt-8 border-t border-subtle pt-4">
              <button
                ...
                className={cn("inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-13 font-medium", confirmDel ? "bg-danger-primary text-white" : "text-danger-primary hover:bg-danger-subtle")}
              >

...
import { BlueprintFlowPanel } from "./blueprint-flow-panel";
...
export const CreateUpdateIssueModalBase = observer(function CreateUpdateIssueModalBase(props: IssuesModalProps) {
  ...
  // BARSOUL: 创建任务 ↔ 发起业务流程 切换(仅创建态)。default single → 现有行为零变化。
  const [createMode, setCreateMode] = useState<"single" | "flow">("single");
  // store hooks
  ...
  const addIssueToCycle = async (issue: TIssue, cid: string) => {
    if (!workspaceSlug || !issue.project_id) return;

    await issues.addIssueToCycle(workspaceSlug.toString(), issue.project_id, cid, [issue.id]);
    fetchCycleDetails(workspaceSlug.toString(), issue.project_id, cid);
  };
  ...
  const addIssueToModule = async (issue: TIssue, moduleIds: string[]) => {
    ...
    await Promise.all([
      ...
      ...moduleIds.map(
        (mid) => issue.project_id && fetchModuleDetails(workspaceSlug.toString(), issue.project_id, mid)
      ),
    ]);
  };
  ...
  const handleModuleChange = async (issueData: Partial<TIssue>, payload: Partial<TIssue>) => {
    ...
    for (const mid of updatedModuleIds) {
      if (issueData.module_ids?.includes(mid)) {
        modulesToRemove.push(mid);
      } else {
        modulesToAdd.push(mid);
      }
    }
  ...
  // BARSOUL: 仅创建态(无既有 id)显示「单发任务 / 业务流程」切换; 选流程 → 蓝图发起面板。
  const zh = currentLocale !== "ja";
  const flowable = !data?.id && !!workspaceSlug && !!activeProjectId;
  return (
    <ModalCore
      isOpen={isOpen}
    ...
    >
      {flowable && (
        <div className="mb-2 flex justify-center">
          <div className="flex items-center gap-0.5 rounded-md bg-layer-2 p-0.5 shadow-raised-100">
            <button
              type="button"
              onClick={() => setCreateMode("single")}
              className={cn(
                "rounded-sm px-3 py-1 text-12 font-medium",
                createMode === "single"
            ...
            <button
              type="button"
              onClick={() => setCreateMode("flow")}
              className={cn(
                "inline-flex items-center gap-1 rounded-sm px-3 py-1 text-12 font-medium",
                createMode === "flow"
                  ? "bg-surface-1 text-primary shadow-raised-100"
                  : "text-secondary hover:text-primary"
              )}
            >
              <LayoutTemplate className="size-3.5" /> {zh ? "业务流程" : "業務フロー"}
            </button>
          </div>
        </div>
      )}
      {flowable && createMode === "flow" ? (
        <BlueprintFlowPanel
          workspaceSlug={workspaceSlug.toString()}
          projectId={activeProjectId ?? ""}
          onClose={handleClose}

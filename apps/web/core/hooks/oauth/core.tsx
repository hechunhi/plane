...
export const useCoreOAuthConfig = (oauthActionText: string): TOAuthConfigs => {
  ...
  const oAuthOptions: TOAuthOption[] = [
    ...
    {
      id: "authelia",
      text: `${oauthActionText} with Authelia`,
      icon: <ShieldCheck size={18} className="text-custom-primary-100" />,
      onClick: () => {
        window.location.assign(`${API_BASE_URL}/auth/authelia/${next_path ? `?next_path=${next_path}` : ``}`);
      },
      enabled: config?.is_authelia_enabled,
    },
  ];

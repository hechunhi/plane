...
def _is_mixed_cn_ja(text):
    ...
    # 中文の地の文の指標: 简体中文専属/高頻字。
    # BS-226 第2波 (2026-06-05 hechun): 句読点「，？！」依存は脆い。ユーザの中文が
    # 「。」(中日共用)で終わる/句読点無しだと取りこぼし → ja 純判定 → 中文まで
    # "翻訳"(改写)される事故。標点ではなく【日文がまず使わない簡体字・常用語】の
    # 共存で判定(将/给/请/帮/报价/当前/其它/对比/展示/折扣… 等)。これらが
    # >=3 個 = 中文の地の文が確実に存在。漢字は中日共通なので専属字のみ数える。
    cn_chars = len(re.findall(
        r"[的了们这那是不在有和与将把给为对你我他她它没么呢吧吗很真还会能要看请帮做想说让跟从向当其它别也都就最较关联报价当前一起展示确认问题方案折扣优惠们]",
        text))
    return cn_chars >= 3

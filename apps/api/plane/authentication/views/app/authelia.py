# Copied + adapted from views/app/gitea.py for Authelia OIDC SSO
...
from plane.authentication.provider.oauth.authelia import AutheliaOAuthProvider
...
class AutheliaOauthInitiateEndpoint(View):
    def get(self, request):
        ...
        try:
            ...
            provider = AutheliaOAuthProvider(request=request, state=state)
...
class AutheliaCallbackEndpoint(View):
    def get(self, request):
        ...
        if state != request.session.get("state", ""):
            exc = AuthenticationException(
                ...
                error_message="AUTHELIA_OAUTH_PROVIDER_ERROR",
            )
            params = exc.get_error_dict()
            if next_path:
        ...
        try:
            provider = AutheliaOAuthProvider(
